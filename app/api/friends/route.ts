...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, slug, display_name, color_primary, color_secondary, color_accent, color_bg, color_text } = body;
    ...
    if (error) {
      ...
      return NextResponse.json({ error: `Failed to create friend: ${error.message}` }, { status: 500 });
    }
